...
For instance, if talkers primarily vary based on groups like gender or regional dialect, listeners can
...
Sociolinguistic research suggests that grouping factors such as gender and dialect that are indeed
...
given phonetic variability. Here we begin to develop and evaluate such a framework, based on the ideal adapter. Specifically, we consider four ways of grouping observed category tokens: by individual talker,
by gender, by dialect, and by both gender and dialect. We assess the consequences of
attending to versus ignoring these different grouping levels during speech perception. We calculate these consequences at two levels:

1. __Informativity__ at the level of cue distributions: the difference between the marginal (or average) distribution across talkers and the corresponding distribution conditioned on each grouping level. 
2. __Utility__ at the level of category recognition: the probability of correct recognition of individual tokens based on classifiers trained on different grouping levels.
...
Mid-Atlantic, North, Midland, South, and West. Our goal is to assess whether considering dialect-level
...
These results are in line with sociolinguistic research: dialect and gender affect the realization of vowels. Going beyond this research, we can now quantify the relative influence of these factors, as well as talker identity.
...
<!-- what's utility vs informativity?? -->

Different cue distributions lead to different optimal phonetic categorization
strategies. But it's possible that this variation does not actually lead to
_errors_ often enough to matter. The _utility_ of a socio-indexical grouping level
depends on how much knowing that grouping reduces comprehension errors.
Figure {@fig:class-plots} shows the probability of correct recognition
under the distributions for each of the grouping levels, as a measure of utility.
...
<!-- spell out this reasoning a bit more-->
We have shown that talker variability in vowels is _structured_, and that dialect is informative about variation in vowel distributions. Moreover, using an ideal observer model, we have showed that knowledge of this structured variabiltiy substantially improves comprehension in the face of talker variability.
...
relative importance of different socio-indexical groupings. For instance, gender, dialect, and other factors might influence consonant contrasts like /b/ vs. /p/ in substantially different ways than vowels.




Moreover, an ideal observer approach, suggests that tracking dialect-level distributions has substantial utility for listeners, by reducing the chance of comprehension errors.  



Nevertheless, talker-specific distributions are more informative and have more utility
than dialect alone.

has substantial utility for listeners.
